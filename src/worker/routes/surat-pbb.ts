...
suratPBBRoutes.post('/', async (c) => {
  try {
    ...
    const { 
      nomor_objek_pajak, nama_wajib_pajak, alamat_wajib_pajak, alamat_objek_pajak,
      luas_tanah, luas_bangunan, nilai_jual_objek_pajak, jumlah_pajak_terhutang,
      tahun_pajak, status_pembayaran, id_dusun
    } = await c.req.json();

    if (!nomor_objek_pajak || !nama_wajib_pajak || !alamat_wajib_pajak || !alamat_objek_pajak ||
        !luas_tanah || !luas_bangunan || !nilai_jual_objek_pajak || !jumlah_pajak_terhutang ||
        !tahun_pajak || !status_pembayaran || !id_dusun) {
    ...
    await c.env.DB.prepare(
      ...
      suratId, nomor_objek_pajak, nama_wajib_pajak, alamat_wajib_pajak, alamat_objek_pajak,
      luas_tanah, luas_bangunan, nilai_jual_objek_pajak, jumlah_pajak_terhutang,
      tahun_pajak, status_pembayaran, id_dusun, user.userId
    ).run();
...
suratPBBRoutes.get('/', async (c) => {
  try {
    ...
    let query = 'SELECT s.*, d.nama_dusun, p.nama_lengkap as nama_perangkat FROM surat_pbb s JOIN dusun d ON s.id_dusun = d.id JOIN pengguna p ON s.id_perangkat_desa = p.id';
    const params: string[] = [];

    if (user.roles !== 'superadmin') {
      const perangkat = await c.env.DB.prepare(
        'SELECT jabatan FROM perangkat_desa WHERE id = ?'
      ).bind(user.userId).first();

      if (perangkat?.jabatan === 'kepala_dusun') {
        const perangkatData = await c.env.DB.prepare(
          'SELECT id_dusun FROM perangkat_desa WHERE id = ?'
        ).bind(user.userId).first();
        
        if (perangkatData && perangkatData.id_dusun) {
          query += ' WHERE s.id_dusun = ?';
          params.push(perangkatData.id_dusun as string);
        }
      } else if (perangkat?.jabatan === 'ketua_rt') {
        const dusunKetua = await c.env.DB.prepare(
          'SELECT id_dusun FROM surat_pbb WHERE id_perangkat_desa = ? LIMIT 1'
        ).bind(user.userId).first();
    ...
    if (status_pembayaran) {
      query += params.length > 0 ? ' AND s.status_pembayaran = ?' : ' WHERE s.status_pembayaran = ?';
