...
aduanRoutes.post('/', authMiddleware, async (c) => {
  try {
    ...
    ).bind(aduanId, judul, isi, kategori, 'baru', user.userId).run();
...
aduanRoutes.get('/saya', authMiddleware, async (c) => {
  try {
    ...
    const aduan = await c.env.DB.prepare(
      'SELECT * FROM aduan WHERE id_masyarakat = ? ORDER BY waktu_dibuat DESC'
...
aduanRoutes.put('/:id/status', authMiddleware, async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      'UPDATE aduan SET status = ?, waktu_diperbarui = datetime("now", "+7 hours", "localtime") WHERE id = ?'
    ).bind(status, aduanId).run();
...
aduanRoutes.post('/:id/tanggapan', authMiddleware, async (c) => {
  try {
    ...
    ).bind(tanggapanId, isi_tanggapan, aduanId, user.userId).run();
